...
function getCredentialUsername(credential) {
  if (credential && typeof credential === 'object') {
    return String(credential.vpnUsername || credential.username || credential.credential || '').trim();
  }
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'generate') {
        ...
        const used = await readBlobData(USED_CREDENTIALS_FILE) || [];
        const usedArray = Array.isArray(used) ? used : [];
        const usedUsernames = new Set(usedArray.map(getCredentialUsername).filter(Boolean));
        
        // Remove eventuais inconsistências (credenciais já usadas que ainda estejam na fila)
        const cleanAvailableArray = availableArray.filter(c => {
          const username = getCredentialUsername(c);
          return username && !usedUsernames.has(username);
        });

        if (cleanAvailableArray.length !== availableArray.length) {
          await writeBlobData(CREDENTIALS_FILE, cleanAvailableArray);
        }

        if (cleanAvailableArray.length === 0) {
          return res.status(400).json({ message: 'Não há credenciais disponíveis' });
        }

        // Sempre usa a primeira credencial da lista (FIFO)
        const credential = cleanAvailableArray[0];
        ...
        const credentialUsername = getCredentialUsername(credentialData);
        const updatedAvailable = cleanAvailableArray.slice(1);
      ...
      return res.status(400).json({ message: 'Ação inválida. Use "upload", "generate" ou "clear-available"' });
    }
